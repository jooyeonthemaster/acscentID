...
## 2026-08-05 - [UI] 용신 배지를 실물 클리커 상판으로 시각화

**Changes**:
- 체크아웃의 둥근 오행 pill을 검정·남청 정오각형 바디, 오행색 이중 림, 한자·상징 선각으로 교체
- 목·화·토·금·수 각각 나무·불꽃·지층·결정·물결 문양을 SVG로 구현
- 단일·다중 체크아웃과 관리자 주문에서 공용 배지를 사용하고, 다중 주문의 용신 표시 누락 보완

---

## 2026-08-05 - [UPDATE] 사주 상품 대표 이미지·공식 명칭 통일

**Changes**:
- 메인 프로그램 카드의 사주 대표 이미지를 상세페이지 첫 실사인 `saju-lineup-square.png`로 교체
- 공식 상품명을 `사주 분석 퍼퓸&디퓨저 클리커 키링`으로 통일하고 5개 로케일·SEO·관리자·알림 폴백 동기화
- 운영 `admin_products` 이름과 `admin_product_images` 대표 행을 즉시 갱신하고 재현용 forward migration 추가

---

## 2026-08-05 - [FIX] 전 상품 단품 10ml 판매가 24,000원 통일

**Changes**:
- `graduation`, `signature`, `saju_perfume`의 잘못된 10ml 판매가(34,000/48,000원)를 24,000원으로 수정
- 코드 폴백을 `STANDARD_PERFUME_10ML_PRICE`로 일원화하고 사주 랜딩·홈·JSON-LD·5개 로케일 가격 문구 동기화
- 기존 장바구니의 10ml 가격 스냅샷을 재가격하고 체크아웃 진입 시 현재 가격표로 재동기화
- 관리자 API 및 DB CHECK로 단품 10ml 가격이 다시 달라지는 것을 차단
- 예외 유지: `payment_test/10ml` 1,000원, `chemistry_set/set_10ml` 10ml×2 세트 44,000원
...
## 2026-07-07 - [ADD] 사주 분석 퍼퓸&디퓨저 클리커 키링 프로그램 전체 구현 (운명의 조향)
