...
`docs/saju/`를 템플릿으로: **DESIGN.md**(식별자 §0 + 아키텍처 + 플로우 + 통합 요약), **CONTENT.md**(해석 프레임워크 — AI 콘텐츠의 심장: 아키타입/서사 규칙/금지 클리셰/톤 견본), **UI-SPEC.md**(디자인 바이블 — **ui-conventions.md §8의 필수 목차 8종을 전부 충족해야 함**: 토큰 WCAG 실측, 컴포넌트 애니메이션 스펙, 화면별 실카피, 초 단위 로딩 스토리보드, 스크롤 바인딩 수치(§9 규약), AI 필드→슬롯 매핑+clamp 표, 인쇄 픽셀 좌표). 도메인 지식이 필요하면 리서치 에이전트를 먼저 (라이브러리 실태는 npm view로 실검증시킬 것).
