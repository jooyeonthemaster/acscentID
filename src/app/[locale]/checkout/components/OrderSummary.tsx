  ...
  return (
    ...
    >
      ...
      <div className="flex gap-4">
        {/* 이미지 */}
        <div className="w-24 h-28 flex-shrink-0 rounded-[12px] overflow-hidden bg-[#151823] border-2 border-[#262A38]">
          {userImage ? (
            <img
              src={userImage}
        ...
        <div className="flex-1 min-w-0 space-y-2">
          <div>
            <p className="text-sm lg:text-base text-[#9F9F9F] font-black tracking-wide">{perfumeBrand}</p>
            <h4 className="font-black text-[#E9E2D0] text-base leading-tight mt-0.5">{perfumeName}</h4>
          </div>
          ...
          {keywords.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {keywords.slice(0, 3).map((keyword, idx) => (
                <span
                  key={idx}
                  className="text-[10px] lg:text-[12px] px-2 py-0.5 bg-[#151823] text-[#E9E2D0] rounded-full font-bold border border-[#262A38]"
                >
                  #{keyword}
                </span>
              ))}
          ...
          <p className="text-xl font-black text-[#E9E2D0]">
            {quantity > 1 ? (
              <>
                {(price * quantity).toLocaleString()}{t('currency.suffix')}
                <span className="text-xs lg:text-sm text-[#8B8578] font-bold ml-1">
      ...
      <div className="space-y-3">
        <p className="text-sm lg:text-base font-black text-[#E9E2D0] flex items-center gap-2">
          {isFigureDiffuser ? (
            ...
            <>
              <Bird size={14} className="text-[#8B8578]" />
              {t('checkout.signaturePerfumeLabel')}
            </>
          ) : isStoreProduct ? (
            <>
              <ShoppingBag size={14} className="text-[#8B8578]" />
              {t('checkout.productSelection')}
            </>
          ) : isChemistrySet ? (
            <>
            ...
            <>
              <Sparkles size={14} className="text-[#9F9F9F]" />
        ...
        {options.length === 0 ? (
          <div className="rounded-[12px] border-2 border-dashed border-[#262A38] p-4 text-center text-sm lg:text-base font-bold text-[#8B8578]">
            {t('checkout.sizeSelection')}
          </div>
        ) : options.length === 1 ? (
          /* 단일 옵션 — 카드형 (항상 선택됨) */
          (() => {
            ...
            return (
              <div className="grid grid-cols-1 gap-3">
                <div className="relative p-4 rounded-[12px] border-2 border-[#262A38] bg-gradient-to-br from-[#151823] to-[#0C0E16]">
                  <div className="absolute -top-2 -right-2 w-6 h-6 bg-[#161925] rounded-full border-2 border-[#262A38] flex items-center justify-center">
                    <Check size={12} className="text-[#E9E2D0]" strokeWidth={3} />
                  </div>
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      {option.image_url && (
                        <img
                          ...
                          alt={optionLabel}
                          className="w-12 h-12 rounded-[12px] border-2 border-[#262A38] object-cover bg-[#12141D]"
                        />
                      )}
                      <p className="font-black text-[#E9E2D0] text-lg truncate">{optionLabel}</p>
                    </div>
                    <div className="flex items-end gap-2 shrink-0">
                      <p className="text-xl font-black text-[#E9E2D0]">{formatPrice(option.price)}{t('currency.suffix')}</p>
                      {option.original_price && option.original_price > option.price && (
                        <>
                          <span className="text-xs lg:text-sm text-[#8B8578] line-through">{formatPrice(option.original_price)}{t('currency.suffix')}</span>
                          {discount !== null && (
                            <span className="px-1.5 py-0.5 bg-[#161925] text-[#E9E2D0] text-[9px] font-black rounded-[12px]">{discount}% OFF</span>
                          )}
                        </>
                      )}
                    </div>
            ...
            {options.map((option) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex h-full min-h-[172px] flex-col rounded-[12px] border-2 p-2.5 text-left transition-all ${
                    selected
                      ? "border-[#262A38] bg-[#151823]"
                      : "border-[#262A38] bg-[#12141D] hover:border-[#262A38]"
                  }`}
                >
                  {selected && (
                    <div className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-[#161925] rounded-full border-2 border-[#262A38] flex items-center justify-center">
                      <Check size={10} className="text-[#E9E2D0]" strokeWidth={3} />
                    </div>
                  )}
                  <div className="mb-2 aspect-square w-full overflow-hidden rounded-[12px] border border-[#262A38] bg-[#12141D]">
                    {option.image_url ? (
                      <img
                        src={option.image_url}
                        alt={optionLabel}
                      ...
                      <div className="flex h-full w-full items-center justify-center bg-[#151823]">
                        <Package size={20} className="text-[#5C564A]" />
                      </div>
                    )}
                  </div>
                  <div className="flex flex-1 flex-col">
                    <div>
                      <p className="text-[15px] font-black leading-tight text-[#E9E2D0]">{optionLabel}</p>
                      <p className="mt-0.5 text-sm lg:text-base font-black leading-tight text-[#E9E2D0]">
                        {formatPrice(option.price)}{t('currency.suffix')}
                      </p>
                      {option.original_price && option.original_price > option.price ? (
                        <span className="mt-1 block text-[10px] lg:text-[12px] leading-none text-[#8B8578] line-through">
                    ...
                    <div className="mt-1">
                      {discount !== null ? (
                        <span className="inline-block rounded-[12px] bg-[#161925] px-1.5 py-0.5 text-[9px] font-black text-[#E9E2D0]">
                          {discount}% OFF
                        </span>
                      ) : (
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between bg-[#151823] rounded-[12px] p-3 border-2 border-[#262A38]">
          <div className="flex items-center bg-[#12141D] rounded-[12px] border-2 border-[#262A38]">
            <button
              onClick={() => onQuantityChange(Math.max(1, quantity - 1))}
              className="w-10 h-10 flex items-center justify-center text-[#8B8578] hover:text-[#A69F8D] transition-colors disabled:opacity-30"
              disabled={quantity <= 1}
            >
              <Minus size={16} />
            </button>
            <span className="w-10 text-center font-black text-lg text-[#E9E2D0]">{quantity}</span>
            <button
          ...
          <span className="font-black text-xl text-[#E9E2D0]">
            {(price * quantity).toLocaleString()}{t('currency.suffix')}
          </span>
        </div>
        {isRepurchaser === true && (
      ...
      <div className={`${isScentPaper ? 'bg-[#151823]' : isFigureDiffuser ? 'bg-[#151823]' : isGraduation ? 'bg-[#151823]' : isSignature ? 'bg-[#151823]' : isStoreProduct ? 'bg-[#151823]' : isChemistrySet ? 'bg-[#151823]' : 'bg-[#DDDDDD]'} border-2 border-[#262A38] rounded-[12px] p-4`}>
        <p className="text-sm lg:text-base font-black text-[#E9E2D0] mb-3">{t('checkout.included')}</p>
        <ul className="space-y-2 text-sm lg:text-base text-[#E9E2D0] font-bold">
          {isScentPaper ? (
            ...
            <>
              ...
              <li className="flex items-center gap-2">
                <span className="w-4 h-4 rounded-full bg-[#12141D] border border-[#262A38] flex items-center justify-center text-[10px] lg:text-[12px]">✓</span>
                {isFreeShippingPromo
                  ? <span className="text-[#A69F8D] font-bold">{t('checkout.freeShippingLabel')} ({t('checkout.eventLabel')})</span>
