  ...
  return (
    <>
      <div className="bg-[var(--soft)] border border-[var(--line)] rounded-[6px] p-4 mb-4">
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="relative shrink-0 rounded-full focus:outline-none focus:ring-2 focus:ring-[var(--line)]"
          >
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={avatar}
              alt={t('profile.profileAlt')}
              className="w-14 h-14 rounded-full object-cover border border-[var(--line)] bg-[var(--soft)]"
            />
            {unreadCount > 0 && (
              <span className="absolute -top-1.5 -right-1.5 min-w-[20px] h-5 px-1 flex items-center justify-center rounded-full bg-red-500 border border-[var(--line)] text-[var(--ink)] text-[11px] lg:text-[13px] font-black leading-none">
                {unreadCount > 9 ? '9+' : unreadCount}
              </span>
            )}
          </button>
          <div className="flex-1 min-w-0">
            <h2 className="text-base font-bold text-[var(--ink)] truncate">{currentName}</h2>
            {email && <p className="text-xs lg:text-sm text-[var(--muted-ink)] truncate">{email}</p>}
            <span className="inline-flex mt-1 text-[10px] lg:text-[12px] px-2 py-0.5 bg-[var(--soft)] text-[var(--muted-ink)] rounded-full">
      ...
      {editing && (
        ...
        >
          <div
            className="bg-[var(--paper)] border border-[var(--line)] rounded-[6px] w-full max-w-sm overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between px-5 py-4 border-b-2 border-[var(--line)]">
              <h3 className="text-lg font-bold text-[var(--ink)]">{t('profileEditTitle')}</h3>
              <button
                onClick={() => !saving && setEditing(false)}
            ...
            <div className="px-5 py-5 space-y-4">
              ...
              <div>
                <label className="block text-sm lg:text-base font-bold text-[var(--muted-ink)] mb-1.5">{t('profileNameLabel')}</label>
                <input
                  type="text"
                  value={name}
                  onChange={(e) => {
                  ...
                  autoFocus
                  className="w-full px-4 py-2.5 rounded-[6px] border border-[var(--line)] focus:border-[var(--line)] text-sm lg:text-base transition-colors outline-none"
                />
              </div>

              <div>
                <label className="block text-sm lg:text-base font-bold text-[var(--muted-ink)] mb-1.5">{t('profileEmailLabel')}</label>
                <input
                  type="text"
                  value={email || '-'}
                  disabled
                  className="w-full px-4 py-2.5 rounded-[6px] border border-[var(--line)] bg-[var(--soft)] text-sm lg:text-base text-[var(--muted-ink)] cursor-not-allowed"
                />
                <p className="text-xs lg:text-sm text-[var(--muted-ink)] mt-1">{t('profileEmailReadonly')}</p>
              </div>
            </div>

            <div className="flex items-center justify-end gap-2 px-5 py-4 border-t-2 border-[var(--line)] bg-[var(--soft)]">
              <button
