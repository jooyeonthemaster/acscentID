      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className={`fixed inset-x-0 bottom-0 z-50 max-h-[85vh] ${saju ? SJ.sheet : 'bg-white'} rounded-t-3xl shadow-2xl overflow-hidden flex flex-col`}
          >
            {/* 헤더 */}
            <div className={`flex items-center justify-between px-5 py-4 border-b flex-shrink-0 ${saju ? SJ.hairline : 'border-slate-100'}`}>
              {showRecipeConfirm && selectedFeedback ? (
                <>
                  <button
                    ...
                    className={`p-2 -ml-2 rounded-full transition-colors ${saju ? SJ.iconHover : 'hover:bg-slate-100'}`}
                  >
                  ...
                  <div className="flex-1 text-center">
                    ...
                    <p className={`text-xs ${saju ? SJ.inkMuted : 'text-slate-500'}`}>{selectedFeedback.perfumeName}</p>
                  </div>
                ...
                <>
                  <div className="flex items-center gap-3">
                    {saju ? (
                      <span aria-hidden className="flex h-9 w-9 items-center justify-center rounded-[4px] border-[1.5px] border-[#C0392B] text-[#A93226] font-serif-kr text-lg leading-none">錄</span>
                    ) : (
                      <span className="text-2xl">📋</span>
                    )}
                    <div>
                      <h2 className={`text-lg font-bold ${saju ? `${SJ.serif} ${SJ.ink}` : 'text-slate-900'}`}>{t('historyTitle')}</h2>
                      <p className={`text-xs ${saju ? SJ.inkMuted : 'text-slate-500'}`}>{t('historySubtitle')}</p>
                    </div>
                  </div>
                  <button
                    onClick={onClose}
                    className={`p-2 -mr-2 rounded-full transition-colors ${saju ? SJ.iconHover : 'hover:bg-slate-100'}`}
                  >
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4">
              ...
              {!showRecipeConfirm && loading && (
                <div className="flex flex-col items-center justify-center py-12">
                  <Loader2 size={32} className={`animate-spin mb-3 ${saju ? 'text-[#C9A227]' : 'text-amber-500'}`} />
                  <p className={`text-sm ${saju ? SJ.inkMuted : 'text-slate-500'}`}>{t('historyLoading')}</p>
                </div>
              )}
              ...
              {!showRecipeConfirm && error && !loading && (
                <div className="flex flex-col items-center justify-center py-12">
                  ...
                  <p className={`font-medium ${saju ? SJ.inkMuted : 'text-slate-600'}`}>{error}</p>
                </div>
              )}
              ...
              {!showRecipeConfirm && !loading && !error && feedbacks.length === 0 && (
                <div className="flex flex-col items-center justify-center py-12">
                  ...
                  <h3 className={`text-lg font-bold mb-2 ${saju ? `${SJ.serif} ${SJ.ink}` : 'text-slate-700'}`}>{t('historyEmptyTitle')}</h3>
                  <p className={`text-sm text-center whitespace-pre-line ${saju ? SJ.inkMuted : 'text-slate-500'}`}>
                ...
                <div className="space-y-3">
                  {feedbacks.map((feedback) => (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className={`rounded-2xl overflow-hidden ${saju ? 'bg-[#EDE5D2]/70 border border-[#D8CFBB]' : 'bg-slate-50'}`}
                    >
                      {/* 카드 헤더 */}
                      <button
                        onClick={() =>
                        ...
                        className={`w-full px-4 py-3 flex items-center gap-3 text-left transition-colors ${saju ? 'hover:bg-[#EDE5D2]' : 'hover:bg-slate-100'}`}
                      >
                        {/* 향수 아이콘 */}
                        <div
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className={`font-bold truncate ${saju ? `${SJ.serif} ${SJ.ink}` : 'text-slate-800'}`}>
                            {feedback.perfumeName}
                          </h4>
                          <div className={`flex items-center gap-2 text-xs ${saju ? SJ.inkMuted : 'text-slate-500'}`}>
                            <Clock size={12} />
                        ...
                        >
                          <ChevronRight size={20} className={saju ? 'text-[#8B8578]' : 'text-slate-400'} />
                        </motion.div>
                      </button>
                        ...
                        {expandedId === feedback.id && feedback.generatedRecipe && (
                          ...
                          >
                            <div className="px-4 pb-4 space-y-3">
                              {/* 레시피 설명 */}
                              <div className={`rounded-xl p-3 border ${saju ? 'bg-[#FDFAF1] border-[#C9A227]/40' : 'bg-gradient-to-br from-yellow-50 to-amber-50 border-yellow-200/50'}`}>
                              ...
                              <div className="space-y-2">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className={`text-xs font-bold ${saju ? `${SJ.serif} ${SJ.ink}` : 'text-slate-600'}`}>{t('recipeComposition')}</span>
                                </div>
                                <div className="flex flex-wrap gap-2">
                                  {(feedback.generatedRecipe as GeneratedRecipe).granules.map(
                                    (granule, idx) => (
                                      <div
                                        key={idx}
                                        className={`flex items-center gap-2 rounded-lg px-3 py-1.5 border ${saju ? SJ.card : 'bg-white border-slate-100'}`}
                                      >
                                        <div
                                        ...
                                        <span className={`text-xs font-bold ${saju ? SJ.goldText : 'text-amber-600'}`}>
                              ...
                              {(feedback.generatedRecipe as GeneratedRecipe).fanMessage && (
                                <div className={`rounded-xl p-3 border ${saju ? 'bg-[#FDFAF1] border-[#D8CFBB]' : 'bg-gradient-to-r from-pink-50 to-purple-50 border-purple-200/50'}`}>
                                  <p className={`text-xs leading-relaxed ${saju ? `${SJ.serif} ${SJ.inkMuted}` : 'text-purple-700 italic'}`}>
                              ...
                              <div className="flex gap-2">
                                ...
                                <Button
                                  ...
                                  className={`flex-1 h-9 rounded-xl font-medium shadow-md ${
                                    saju ? SJ.ctaCinnabar : 'bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white shadow-green-500/20'
                                  }`}
                                >
            ...
            {/* 푸터 */}
            <div className={`px-5 py-4 border-t flex-shrink-0 ${saju ? `${SJ.hairline} bg-transparent` : 'border-slate-100 bg-white'}`}>
              <Button
                onClick={onClose}
                className={`w-full h-12 rounded-2xl font-bold ${saju ? SJ.ctaInk : 'bg-slate-900 hover:bg-slate-800 text-white'}`}
              >
                {t('closeButton')}
              </Button>
            </div>
