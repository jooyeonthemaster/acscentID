...
export function ChemistryPurchaseChapter({
  ...
  return (
    <div className="px-4">
      <div className="bg-[#F5EFE2] border-2 border-[#D8CFBB] rounded-[12px] overflow-hidden">
        {/* 헤더 — 더 시각적으로 */}
        ...
        <div className="p-5 border-b-2 border-[#1E222E]">
          <div className="flex items-center justify-center gap-4">
            <div className="text-center flex-1">
              <div
                className="w-14 h-14 rounded-full border-2 border-[#D8CFBB] mx-auto"
                style={{ background: perfumeA ? `linear-gradient(135deg, ${perfumeA.primaryColor}, ${perfumeA.secondaryColor})` : '#ddd' }}
              />
              <span className="text-xs lg:text-sm font-bold text-[#1A1610] mt-1.5 block">{perfumeAName || t('chemistry.fallback.perfumeA')}</span>
              <span className="text-[10px] lg:text-[12px] text-[#8B8578] font-bold">{character1Name}</span>
            </div>
            <div className="text-center">
              <div className="w-10 h-10 rounded-full bg-gradient-to-r from-[#D8CFBB] to-[#D8CFBB] border-2 border-[#D8CFBB] flex items-center justify-center">
                <span className="text-base font-black text-[#1A1610]">+</span>
              </div>
              <span className="text-[9px] font-bold text-[#8B8578] mt-1 block">SET</span>
            </div>
            <div className="text-center flex-1">
              <div
                className="w-14 h-14 rounded-full border-2 border-[#D8CFBB] mx-auto"
              ...
              />
              <span className="text-xs lg:text-sm font-bold text-[#1A1610] mt-1.5 block">{perfumeBName || t('chemistry.fallback.perfumeB')}</span>
              <span className="text-[10px] lg:text-[12px] text-[#8B8578] font-bold">{character2Name}</span>
            </div>
          </div>
        </div>
          ...
          {pricing.map((option) => (
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-sm lg:text-base font-black text-[#1A1610]">{option.label}</span>
                </div>
                <span className="text-base font-black text-[#5C564A]">{formatPrice(option.price)}{t('currency.suffix')}</span>
              </div>
            </motion.button>
          ))}
        </div>
        ...
        <div className="p-5 pt-0 space-y-2.5">
          <motion.button
            ...
            disabled={isAdding}
            className="w-full h-14 rounded-[12px] font-black text-base flex items-center justify-center gap-2 bg-[#EFE4C8] text-[#1A1610] border-2 border-[#D8CFBB] transition-all disabled:opacity-50"
          >
          ...
          <motion.button
            ...
            disabled={isAdding}
            className="w-full h-12 rounded-[12px] font-bold text-sm lg:text-base flex items-center justify-center gap-2 bg-[#EFE4C8] text-[#1A1610] border-2 border-[#D8CFBB] hover:bg-[#EFE4C8] transition-all disabled:opacity-50"
          >
            <CreditCard size={16} />
