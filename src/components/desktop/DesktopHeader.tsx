  ...
  return (
    <div className="flex min-w-[240px] flex-col gap-1 p-2">
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="h-9 animate-pulse rounded-[10px] bg-[#1B1F2C]" />
      ))}
    </div>
  )
}
    ...
    <div className="flex min-w-[240px] flex-col gap-0.5 p-2">
      {links.map((link) => (
        <Link
          ...
          href={link.href}
          onClick={onNavigate}
          className="rounded-[10px] px-3 py-2.5 text-sm lg:text-base font-bold text-[#A69F8D] transition-colors hover:bg-[#1B1F2C] hover:text-[#E9E2D0]"
        >
          {t(link.labelKey)}
        </Link>
      ))}
  ...
  return (
    <div className="flex min-w-[200px] flex-col gap-0.5 p-2">
      ...
      <Link
        href="/products"
        onClick={onNavigate}
        className="mt-0.5 border-t border-[#262A38] px-3 pb-1 pt-2.5 text-xs lg:text-sm font-bold text-[#8B8578] transition-colors hover:text-[#E9E2D0]"
      >
        {t('store.viewAll')}
      </Link>
    </div>
    ...
    >
      <button
        ...
        className={cn(
          ...
          open ? 'text-[#E9E2D0]' : 'text-[#A69F8D] hover:text-[#E9E2D0]',
        )}
      >
        {label}
      ...
      {open && (
        <div className="absolute left-0 top-full pt-2">
          <div className="rounded-[12px] border-2 border-[#262A38] bg-[#12141D]">
            {children}
          </div>
        </div>
      )}
...
export function DesktopHeader() {
  ...
  const navLinkClass = (active: boolean) =>
    cn(
      ...
      active ? 'text-[#E9E2D0]' : 'text-[#A69F8D] hover:text-[#E9E2D0]',
    )

  return (
    <>
      <header className="fixed inset-x-0 top-0 z-50 hidden flex-col border-b border-[#262A38] bg-[#0C0E16] lg:flex">
        {/* 마키 티커 — 모바일 헤더와 동일 마크업/높이(28px) */}
        <div className="flex h-7 w-full items-center overflow-hidden border-b border-[#262A38] bg-[#12141D] py-1">
          <div className="animate-ticker flex items-center gap-6 whitespace-nowrap text-[9px] font-black uppercase tracking-[0.15em] text-[#A69F8D]">
        ...
        <div className="mx-auto flex h-14 w-full max-w-[1200px] items-center gap-10 px-6">
          ...
          <Link href="/" aria-label={t('nav.home')} className="flex flex-col items-center">
            ...
            />
            <span className="mt-[3px] text-[7px] font-bold tracking-[0.25em] text-[#A69F8D]">
              IDENTITY
            </span>
          </Link>
          ...
          <div className="flex items-center gap-2">
            ...
            {!loading && currentUser ? (
              <Link
                ...
                className="h-8 w-8 overflow-hidden rounded-full border-2 border-[#A69F8D]/70 bg-[#12141D] transition-all hover:border-[#E9E2D0] active:scale-90"
              >
                {(unifiedUser?.avatar_url || user?.user_metadata?.avatar_url) ? (
                  ...
                  <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-stone-800 to-stone-800">
              ...
              <button
                ...
                className="flex items-center gap-1.5 whitespace-nowrap rounded-full border-2 border-[#5C564A] bg-white/10 px-3 py-1.5 transition-colors hover:bg-white/20"
              >
                <User size={13} className="text-[#A69F8D]" />
                <span className="text-[11px] lg:text-[13px] font-bold text-[#A69F8D]">{t('nav.login')}</span>
              </button>
            ) : null}
            <button
              onClick={() => router.push('/mypage?tab=cart')}
              className="relative flex h-9 w-9 items-center justify-center rounded-full border-2 border-[#262A38] bg-[#12141D] transition-all hover:border-[#A69F8D] hover:bg-[#A69F8D]/10 active:scale-90"
              aria-label={t('nav.cart')}
            >
              <ShoppingCart size={16} className="text-[#E9E2D0]" strokeWidth={2.5} />
              {cartCount > 0 && (
