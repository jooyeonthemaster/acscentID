...
export function StickerPerfume({ className, style, onClick }: StickerProps) {
    return (
        <svg
            ...
            className={`drop-hover:drop-transition-all duration-300 ${className}`}
            style={style}
            onClick={onClick}
        >
            {/* Bottle Body */}
            <path d="M50 80 H150 V160 C150 176.569 136.569 190 120 190 H80 C63.4315 190 50 176.569 50 160 V80 Z" fill="#9F9F9F" stroke="black" strokeWidth="4" />
            {/* Liquid Shine/Reflection */}
            <path d="M60 90 H100 V160 C100 165 95 170 90 170 H60 V90 Z" fill="white" fillOpacity="0.3" />
            <path d="M110 100 H140 V150 C140 155 135 160 130 160 H110 V100 Z" fill="#888888" fillOpacity="0.2" />

            {/* Neck */}
            <rect x="85" y="50" width="30" height="30" fill="#D7D7D7" stroke="black" strokeWidth="4" />
            ...
            {/* Cap (Round) */}
            <circle cx="100" cy="30" r="20" fill="#D7D7D7" stroke="black" strokeWidth="4" />
            ...
            <path d="M90 135 H110" stroke="#9F9F9F" strokeWidth="3" strokeLinecap="round" />
        </svg>
    )
}

// 2. Retro Ticket Sticker
export function StickerTicket({ title, discount, color = "#1B1F2C", className, onClick }: StickerProps & { title: string, discount: string, color?: string }) {
    return (
        <div
        ...
        >
            {/* Ticket SVG Shape */}
            <svg width="260" height="140" viewBox="0 0 260 140" fill="none" className="drop-group-hover:drop-transition-all duration-300">
                <path
            ...
            <div className="absolute inset-0 p-6 flex items-center">
                <div className="w-[170px]">
                    <div className="text-xs lg:text-sm font-black text-[#E9E2D0] tracking-widest mb-1">특별 혜택</div>
                    <div className="text-xl font-black text-[#E9E2D0] uppercase leading-none mb-2 break-keep">{title}</div>
                    <div className="inline-block bg-[#0C0E16] text-[#E9E2D0] px-2 py-0.5 text-xs lg:text-sm font-bold rounded-[12px]">쿠폰</div>
                </div>
                <div className="flex-1 flex justify-center items-center rotate-90">
                    <span className="text-3xl font-black text-[#E9E2D0] whitespace-nowrap">{discount}</span>
                </div>
            </div>
        </div>
    )
...
export function StickerNew({ className }: StickerProps) {
    return (
        <div className={`relative w-24 h-24 flex items-center justify-center ${className}`}>
            ...
            <span className="relative z-10 font-black text-[#E9E2D0] text-xl rotate-[-10deg]">신규!</span>
        </div>
    )
}
...
export function StickerCartridge({ title, subtitle, color, className, onClick }: any) {
    return (
        <div
            onClick={onClick}
            className={`
                relative w-64 h-80 ${color} border-4 border-[#262A38] rounded-t-[12px] rounded-b-[12px]
                
                transition-all cursor-pointer flex flex-col overflow-hidden ${className}
            `}
        >
            {/* Grip Lines */}
            <div className="h-6 bg-black/10 w-full border-b-4 border-[#262A38] flex justify-center gap-2 items-center">
                <div className="w-40 h-1 bg-black/20 rounded-full" />
                <div className="w-40 h-1 bg-black/20 rounded-full" />
            </div>

            {/* Label Area */}
            <div className="flex-1 p-4 flex flex-col items-center justify-center text-center bg-[#12141D] m-3 mb-8 border-2 border-[#262A38] rounded-[12px] relative overflow-hidden">
                {/* Holo Shine */}
                <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/40 to-transparent w-[200%] h-full animate-shine" />

                <h3 className="font-black text-5xl md:text-2xl text-[#E9E2D0] leading-none mb-2 z-10 break-keep">{title}</h3>
                <p className="text-xl md:text-xs font-bold text-[#8B8578] z-10 break-keep">{subtitle}</p>
                <div className="mt-4 z-10">
                    <span className="inline-block px-4 py-2 md:px-2 md:py-1 bg-[#0C0E16] text-[#E9E2D0] text-lg md:text-[10px] font-bold rounded-[12px]">PPUDUCK</span>
                </div>
            </div>

            {/* Bottom Connector */}
            <div className="h-4 bg-[#161925] w-3/4 mx-auto rounded-t-[12px] mb-0" />
        </div>
    )
}
