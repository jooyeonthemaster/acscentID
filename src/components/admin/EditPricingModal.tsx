...
import { AlertTriangle, Check, Eye, EyeOff, ImageIcon, Loader2, Trash2, Upload, X } from 'lucide-react'
...
export function EditPricingModal({
  ...
  row: PricingRow
  onSave: (payload: PricingSavePayload) => Promise<void>
  onClose: () => void
}) {
  ...
  const handleImageFile = async (file: File) => {
    ...
    setErr('')
    try {
      const url = await uploadPricingImage(file, row.product_type)
      setImageUrl(url)
    } catch (e) {
  ...
  const isUnchanged =
    ...
    (imageUrl ?? null) === (row.image_url ?? null)
