...
// ------------------------------------------------------------
//  이 사이트의 관리자 API 는 전부 쿠키 세션 + 이메일 화이트리스트다
//  (src/lib/auth/require-admin.ts). 본사 ERP 는 브라우저가 아니라 서버라
//  쿠키를 들고 올 수 없으므로, 기계용 통로를 따로 둔다.
//
//  ⚠️ 그래서 이 토큰은 **관리자 로그인과 같은 무게**다. 새면 온라인 주문
//     매출이 통째로 읽힌다. 다음을 지킨다:
//       · 값은 환경변수에만 둔다 (ERP_FEED_TOKEN). 코드에 기본값을 두지 않는다.
//       · 설정돼 있지 않으면 라우트는 **열리지 않는다** (503). 빈 토큰이
//         우연히 통과하는 사고를 막는다.
//       · 읽기 전용이다. 이 통로로는 아무것도 쓰지 못한다.
//       · 길이가 같을 때 한 글자씩 비교하지 않는다 (timingSafeEqual).
// ============================================================
