...
// 정책(policy)은 주입 가능 — 서버는 DB(reservation_settings) 값을, 미주입 시 기본값을 쓴다.

import { RESERVATION_CONFIG, type SlotPolicy } from './config'
...
export function isClosedDate(
  dateStr: string,
  policy: SlotPolicy = RESERVATION_CONFIG
): boolean {
  return policy.closedWeekdays.includes(getWeekday(dateStr))
}
...
export function generateSlotsForDate(
  ...
  const open = toMinutes(policy.openTime)
  const close = toMinutes(policy.closeTime)
  const { slotIntervalMinutes, durationMinutes } = policy
 ...
 * 형식(+09:00 고정) / 그리드 / 영업시간 / 휴무일 / 리드타임 / 최대 예약 가능일 모두 통과해야 true.
 */
export function isValidSlot(
  ...
  const open = toMinutes(policy.openTime)
  const close = toMinutes(policy.closeTime)
  const { slotIntervalMinutes, durationMinutes, minLeadTimeHours } = policy
  ...
  // 휴무일
  if (isClosedDate(dateStr, policy)) return false
  ...
  const { endDate } = getBookableDateRange(nowMs, policy)
